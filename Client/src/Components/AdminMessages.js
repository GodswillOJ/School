...
const SendUserMail = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('https://personal-site-awu4.onrender.com/api/messageUser',{ fullName, userEmail, description },
        {
          headers: {
            Authorization: `Bearer ${accessToken}`,
  ...
  return (
    <div>
      ...
      <div className="userDashBoard">
        ...
        <div className="AddOrder_inFo" id='Message_sec'>
        <><Link to="/adminMessages"><p className='Send_Mail'>View Mail</p></Link></>
           <div className='Course_inquiry'>
              <h2>Send Website Description</h2>
              <div id='myOrder_in'>
                <form onSubmit={handleSubmit}>
                  ...
                  <div className='Order_form_in'>
                      <label for="myTextarea">Message details:</label>
                      <textarea id="myTextarea" placeholder="Write message content..." type="text" value={description} onChange={(e) => setDescription(e.target.value)}></textarea>
                  </div>
                  <button type="submit"><h2>Send to User</h2></button>
                </form>
              </div>
           </div>
        </div>
